...
export async function updatePuzzleProgress(puzzleId: string, progressData: any) {
  ...
  try {
    ...
    if (puzzleProgress) {
      ...
      return { puzzleId, done: progressData.completed, message: progressData.completed ? 'Puzzle completed successfully' : 'Puzzle progress updated successfully' };
    } else {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const completePuzzle = async (puzzleId: string, progressData: any) => {
  await connect();
  try {
    const { userId } = await auth();
    const user = await UserModel.findOne({ uid: userId });
    ...
    if (puzzleProgress) {
      ...
      return { puzzleId, done: true, message: 'Puzzle completed successfully' };
    } else {
      throw new Error('Puzzle not found in user progress');
    }
  } catch (error) {
